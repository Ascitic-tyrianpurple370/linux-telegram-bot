...
from time            import sleep,         time,        ctime,    mktime
...
def mount(c, path=None, dev=None, fat=None, opt=None):
    match c:
        case 'get':
            r  = []
            au = r.append
        ...
        case 'mount':            
            if not _exist(dev):
                return f'device does not exist ({dev}) [*]'
            

            if path and not _exist(path):
                try:
                    mkdir(path)
                except OSError:
                    pass
            ...
            with StringIO() as cmb:
                ...
                if opt:
                    cmb.write(f' -o {quote(opt)}')

                cmb.write(f' {quote(dev)}')

                if path: 
                    cmb.write(f' {quote(path)}')
            

                return (
                        f'device is mounted ({dev}) [+]'
                    if shell(cmb.getvalue()) else
                        f'failed to mount device ({dev}) [-]'
                )
        
        case 'umount':
            if not os.path.ismount(path):
                return f'path is not mounted ({path}) [*]'

            return (
                    f'path is unmounted ({path}) [+]'
                if shell(f'umount -l {quote(path)}') else
...
def systeminfo():
    ...
    with StringIO() as buf:
        try:
            idx     = int(_read(f'{PATH_DMI}/chassis_type'))
...
''')
        ...
        bios_vendor  = _read(f'{PATH_DMI}/bios_vendor' )
        bios_version = _read(f'{PATH_DMI}/bios_version')
        bios_date    = _read(f'{PATH_DMI}/bios_date'   )
...
''')
        ...
        baseboard_product = _read(f'{PATH_DMI}/board_name'   )
        baseboard_vendor  = _read(f'{PATH_DMI}/board_vendor' )
        baseboard_version = _read(f'{PATH_DMI}/board_version')
...
''')
        ...
        case 'mount':
            send('''
mount -g  —  Get mounts\n
mount -m (device) -p (none|path) -t (none|filesystem) -o (none|options)  —  Mount device\n
mount -u (path)  —  Unmount path
''')
            return
...
''')
        ...
        case 'journalctl':
            r = shell('journalctl --no-pager', output=True)

            send(r, doc='journalctl.txt') if r else send(NULL)
            return
        ...
        case 'setuid':
            if args.isdigit():
                ...
                try:
                    ...
                    send(f'UID is set {usrn} ({uid}) [+]')
                except OSError:
                    send(f'failed to set {usrn} UID ({uid}) [-]')
...
''')
        ...
        case 'mount':
            ...
            if exp := parse_cmd(r'-m\s*(?P<dev>.*?)\s*-p\s*(?P<path>.*?)\s*-t\s*(?P<fat>.*?)\s*-o\s*(?P<opt>.+)', args):
                dev  = exp['dev' ]
                path = exp['path'] if exp['path'] != 'none' else None
                fat  = exp['fat' ] if exp['fat' ] != 'none' else None
                opt  = exp['opt' ] if exp['opt' ] != 'none' else None

                send(mount('mount', path, dev, fat, opt))
                return
        ...
        case 'inet':
            if args in ('-e', '-d'):
                shell(f'nmcli networking {"on" if args == "-e" else "off"}')
                return
        ...
        case 'service':
            if args == '-g':
                ...
                match exp['mode']:
                    case '-d' : send(systemctl('delete',  name))
                    case '-e' : send(systemctl('enable',  name))
                    case '-i' : send(systemctl('disable', name))
                    case '-l' : send(systemctl('start',   name))
                    case '-r' : send(systemctl('restart', name))
                    case '-s' : send(systemctl('stop',    name))
        ...
        case 'user':
            if args == '-g':
                ...
                send(r, doc='user.txt') if r else send(f'user does not exist ({name}) [*]')
                return
        ...
        case 'block':
            if args == '-l':
                ...
                send(tabulate(
                    ...
                    headers=('NAME', 'DATE'),
                    tablefmt='grid'
                ), doc='blocked_apps.txt') if r else send('no blocked apps [*]')
                return
